...
class LatestRecyclerViewAdapterImpl @Inject constructor(
...
) : LatestRecyclerViewAdapter(pageSize) {
    ...
    override fun updateList(items: LatestList) {
        this.items.clear()
    ...
    override fun onBindViewHolder(holder: InteractiveRecyclerViewHolder, position: Int) {
        holder.onItem<LatestRecyclerViewHolderImpl> {
            bind(items[position])
        }
    }
